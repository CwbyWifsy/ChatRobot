...
- 🌐 **Web 聊天前端**：内置一个轻量的网页对话界面，支持在浏览器中直接与小说问答助手交互。
...
**Milvus 启动方式（docker-compose 推荐）**

本项目已在 milvus/ 目录中提供完整的 docker-compose.yml 配置，以及必需的 volumes/ 数据目录。
你只需要进入该目录并启动服务即可。
...
# 3) 启动 FastAPI 服务
uvicorn app.main:app --host 0.0.0.0 --port 10020 --reload
```
- 服务启动后，有两种体验方式：
   - **方式 A：浏览器前端（推荐）**
     - 打开项目根目录下的 index.html（或使用任意本地静态服务器），确保页面中 API_BASE_URL 指向 http://127.0.0.1:10020，然后在输入框中直接提问即可看到回答和引用章节。
     - **方式 B：命令行 curl**
       ```bash
       curl -X POST http://127.0.0.1:10020/api/chat \
...
- 确认后端已在 http://127.0.0.1:10020 监听（如需修改端口，请同步修改前端中的 API_BASE_URL）。
- 使用浏览器直接打开 index.html（或在 VS Code / PyCharm 中通过 Live Server 打开）。
- 在页面底部输入问题，点击“发送”，即可获得模型回答和对应的小说引用信息。
...
欢迎根据业务需求进一步扩展，如优化 Web 前端、任务队列等功能。
# ChatRobot
